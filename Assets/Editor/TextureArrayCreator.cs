...
public class TextureArrayCreator : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (_textureSource != null)
        {
            ...
            else
            {
                EditorGUILayout.HelpBox(
                    "Ошибка: Выбранный объект не реализует интерфейс ITextureProvider!",
                    MessageType.Error);
            }
        }
        ...
        EditorGUI.BeginDisabledGroup(
            !isValidProvider || string.IsNullOrEmpty(_savePath));

        if (GUILayout.Button(
                "Сгенерировать Texture2DArray",
                GUILayout.Height(30)))
        {
            GenerateArray();
        }
    ...
    private void GenerateArray()
    {
        ...
        for (int i = 0; i < textures.Length; i++)
        {
            ...
            {
                Debug.LogError(
                    $"Текстура по индексу {i} отсутствует!");
                return;
            }
            ...
            {
                Debug.LogError(
                    $"Текстура '{textures[i].name}' имеет другой размер! " +
                    "Все текстуры должны быть одинакового размера.");
            ...
            {
                Debug.LogWarning(
                    $"Текстура '{textures[i].name}' имеет формат " +
                    $"{textures[i].format}, ожидался {format}.");
            }
        }
        ...
        for (int i = 0; i < textures.Length; i++)
        {
            int mipCount = _generateMipMaps
                ? textures[i].mipmapCount
                : 1;
        ...
        Debug.Log(
            $"<color=green>Texture2DArray успешно создан:</color> " +
            $"{_savePath} | MipMaps: {_generateMipMaps}");
    }
}