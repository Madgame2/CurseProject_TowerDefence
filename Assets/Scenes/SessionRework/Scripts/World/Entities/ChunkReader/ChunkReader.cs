...
namespace Scenes.SessionRework.Scripts.World.Entities.Chunk
{
    public class ChunkReader : MonoBehaviour, IChunkReader
    {
        ...
        [SerializeField] private RenderModuleBase[] _renderModules;
        
        [Inject] private readonly IWorldProvider _worldProvider;

        private Model.Chunk _chunk;
        ...
        public void Link(ref Model.Chunk chunkModel)
        {
            _chunk = chunkModel;
            _isLinked = true;

            foreach (var renderModule in _renderModules)
            {
                renderModule.Link(ref _chunk);
                renderModule.Setup();
            }
